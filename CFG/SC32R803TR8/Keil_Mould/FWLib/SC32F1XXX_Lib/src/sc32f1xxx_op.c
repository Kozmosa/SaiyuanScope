 ...
 */
/* Includes ------------------------------------------------------------------*/
#if defined(SC32f12xx) || defined(SC32R805) || defined(SC32R806) || defined (SC32f15xx) || defined (SC32R803)
...
void OP_Init ( OP_TypeDef* OPx, OP_InitTypeDef* OP_InitStruct )
{
    ...
    tmpreg &= ( uint32_t ) ~ ( OP_CON_OPOSEL | OP_CON_OPNSEL | OP_CON_OPPSEL
                               | OP_CON_PGAGAIN | OP_CON_FDBRSEL | OP_CON_PGAOFC );
    /* Set OPFS bit to OP_FreqSelect value */
    ...
    tmpreg &= (uint32_t)~(OP_CON_OPOSEL | OP_CON_OPNSEL | OP_CON_OPPSEL |
                          OP_CON_PGAGAIN | OP_CON_OPRF | OP_CON_FDBRSEL);
 ...
 *                SC32R803 Selection range(OP_PGAGain_NonInvert8_Invert7,OP_PGAGain_NonInvert16_Invert15,OP_PGAGain_NonInvert32_Invert31,OP_PGAGain_NonInvert64_Invert63)
...
void OP_OffsetZero(OP_TypeDef* OPx)
{
    ...
    if(OPx == OP_0)
    {
        ...
        /*!< Configure the new trimming value */
        tmpreg |= (uint32_t)((OP_Nmos_offset << OP_CON_TRIMOFFSETN_Pos) | (OP_Pmos_offset <<OP_CON_TRIMOFFSETP_Pos ));

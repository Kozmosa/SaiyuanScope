...
void UART_SendByte(UART_TypeDef* UARTx,uint8_t Byte)
{
	UART_SendData(UARTx, Byte);		//将字节数据写入数据寄存器，写入后USART自动生成时序波形
	while (UART_GetFlagStatus(UARTx, UART_Flag_TX) == RESET);	//等待发送完成
	/*下次写入数据寄存器会自动清除发送完成标志位，故此循环后，无需清除标志位*/
}
void UART_SendArray(UART_TypeDef* UARTx,uint8_t *Array, uint16_t Length)
{
	uint16_t i;
	for (i = 0; i < Length; i ++)		//遍历数组
	{
		UART_SendByte(UARTx,Array[i]);		//依次调用Serial_SendByte发送每个字节数据
		delay_ms(1000);
	}
}
void UART_SendPacket()
{
	
	UART_SendArray(UART3,UART3_TxPacket,4);
}
void UART_SendString(UART_TypeDef* UARTx,char *String)
{
	...
	{
		UART_SendByte(UART3,String[i]);		//依次调用Serial_SendByte发送每个字节数据
		delay_ms(1000);
	}
}
/**
...
#elif defined(SC32f12xx)||defined(SC32R805) || defined(SC32R806) ||defined(SC32f15xx)||defined(SC32R601)
    tmpreg = UARTx->UART_CON;
 ...
 *                   SC32R803 Selection range(UART_IT_EN,UART_IT_TX,UART_IT_RX,UART_IT_BK,UART_IT_SL,UART_IT_SV)
 ...
 *                   SC32R803 Selection range(UART_Flag_RX,UART_Flag_TX,UART_Flag_BK,UART_Flag_SY)
 ...
 *                   SC32f10xx Selection range(UART_Flag_RX,UART_Flag_TX)
 *                   SC32R803 Selection range(UART_Flag_RX,UART_Flag_TX,UART_Flag_BK,UART_Flag_SY,UART_Flag_SLVYN,UART_Flag_SLVHE)
 *                   SC32f12xx Selection range(UART_Flag_RX,UART_Flag_TX,UART_Flag_BK,UART_Flag_SY,UART_Flag_SLVYN,UART_Flag_SLVHE)
 *                   SC32f15xx Selection range(UART_Flag_RX,UART_Flag_TX,UART_Flag_BK,UART_Flag_SY,UART_Flag_SLVYN,UART_Flag_SLVHE)
 *                  - UART_Flag_RX:Receive flag
 *                  - UART_Flag_TX:Transmit flag
 *                  - UART_Flag_BK:Break flag
 *                  - UART_Flag_SY:SYNCIE flag
...
/* End of UART_Group3.	*/
#if defined(SC32f12xx)||defined(SC32R805) || defined(SC32R806)|| defined(SC32f15xx)|| defined(SC32R803)|| defined(SC32R601)
